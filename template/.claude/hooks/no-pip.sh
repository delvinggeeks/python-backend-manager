#!/usr/bin/env bash
# PreToolUse(Bash) hook: block raw pip installs — this service manages deps with uv.
...
if printf '%s' "$cmd" | grep -Eq '(^|[;&|]|&&|\|\|)[[:space:]]*(sudo[[:space:]]+)?(uv[[:space:]]+pip|pip3?|python3?[[:space:]]+-m[[:space:]]+pip)[[:space:]]+install'; then
  cat <<'JSON'
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Use `uv add` instead of pip — this service manages dependencies with uv (a raw pip install does not update pyproject.toml / uv.lock). Try `uv add <pkg>`, `uv add --optional <extra> <pkg>`, or `uv add --dev <pkg>`."}}
JSON
  exit 0
fi
exit 0
