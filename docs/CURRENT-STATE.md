...
| **agents** | One agent framework per service (`pydantic-ai`/`langgraph`/`openai-agents`), model tiers from settings (`model_fast`/`model_default`/`model_frontier`), a thin `example_agent.py` runner + `/agent` route, `llm` extra (anthropic/openai/litellm/instructor/tiktoken), `rag` extra (deps only), FastMCP toggle. | Frameworks are conflicting extras; never two. **The AI *application layer* (gateway/token-metering, RAG module, memory, evals, guardrails) is a base only** — fully specced in [AI-AGENTIC-STACK.md](AI-AGENTIC-STACK.md). |
