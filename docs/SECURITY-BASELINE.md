...
| 3 | **Every tenant table is RLS-protected, or exempt by name** | **policy/CI** | `tests/test_rls.py::test_every_tenant_table_is_rls_protected` — enumerates every `organization_id`-bearing table from SQLAlchemy metadata, applies the real migration chain to Postgres, and requires each table to have a policy AND `FORCE`, or an entry in `RLS_EXEMPT_TABLES` with a stated reason. Metadata is the source of truth, so a new model is covered the moment it exists. A companion test rejects stale exemptions, scoped to capabilities the render actually ships. The enumeration **fails closed** (`_absent_capability`): it skips a model module only when the missing module is that module itself or a package on its path — i.e. this render omits the capability — and raises on any other import failure. Guarding both cases with one `contextlib.suppress(ImportError)` made the gate shrink instead of erroring; measured on a metering render, one module whose dependency was missing left **1 of 5** tenant tables checked, the four money tables of the row below unverified, and the gate green |
| 3 | **Money and outbox rows are covered by RLS, not by app scoping alone** | **environment** (Postgres policy) | `migrations/versions/0013_rls_backfill.py` protects the tenant tables created *after* `0008_rls` — `usage_events`, `invoices`, `customer_wallets`, `wallet_transactions`, `outbox_events` — with the identical policy, so there is one shape to audit rather than two. Ordering, not oversight, is why they were unprotected: those tables do not exist yet when 0008 runs. `wallet_transactions` is protected on a **denormalised** `organization_id` added by `0012_wallet_org` (expand→backfill→contract) rather than an `EXISTS` against its parent wallet, which would put a per-row subquery on the highest-write table in the schema. `RLS_EXEMPT_TABLES` now holds only `memberships`, the one exemption that is design rather than debt |
...
| 12 | **The weekly harvest cannot fail silently — nor succeed dishonestly** | **policy/CI** | `.github/workflows/gc-friday.yml` opens the GC-Friday inventory issue; #77 is the first that ever opened, the job having aborted on every prior invocation. Under `set -euo pipefail` a reader that closes early (`head`, `grep -q`) SIGPIPEs its writer, `pipefail` promotes that to 141 and `-e` kills the step. The second failure is the one to remember: **a SIGPIPE inside an `if` condition does not trip `-e` — it just evaluates false**, so the AGENTS.md section reported `_unchanged_` on precisely the weeks it *had* changed, and **exited 0** doing it. **Green-but-wrong is the class**: the crash announced itself, the lie did not, and only the lie would have survived a passing run. Readers now consume their whole input (`sed -n`, command substitution); verified in both directions, and by dispatch rather than by reading |
| — | **The formatter that edits code and the gates that judge it cannot run different ruffs** | **environment** (agent harness) | `.ruff-version` is the only place this repo records a ruff version, and all three consumers read that one file: `.claude/hooks/ruff-format.sh` (PostToolUse), `.github/workflows/ci.yml` job `repo-lint`, and `scripts/micro-render-check.py` via `.pre-commit-config.yaml`. None resolves a `ruff` from `PATH`, and the hook leaves a file **unformatted with a message** when the pinned ruff cannot be fetched, because an unformatted file is a loud CI failure while a wrongly-formatted one is a silent bad diff. The failure was measured, not imagined: both of the hook's old arms resolved **0.4.10** — the manager repo has no Python project, so `uv run ruff` fell through to the same PATH binary — and one run over `tests/test_rls.py` rewrote the assert in `test_rls_exemptions_are_not_stale`, **~75 lines from any edit**, into 0.4.10's pre-parenthesized-message style; the rendered `tenancy` leg's own ruff 0.16.0 then failed `ruff format --check` on exactly that assert. **Position is the point:** the drift is not *detected* by a gate comparing two pins, it is unreachable, because there is only one version to read. Two of the three consumers had been resolving their own, and the old inline `RUFF_VERSION=` was tracked by nothing — its `# renovate:` marker matched no manager, custom or built-in. **The residual, and its bound.** A capability leg resolves ruff from a fresh `uv lock` against the template's `ruff>=0.15` floor, so legs float to whatever is newest at run time and the pin can only ever be *behind* them. The legs keep floating by design — a leg on the newest ruff is how this repo learns that a new ruff broke the template, the signal that caught 0.16's markdown formatting — so the lag is **bounded, not gated**: `renovate.json`'s last `packageRule` gives `.ruff-version` a 1-day cooldown plus `schedule: ["at any time"]`, automerged on minor/patch only, and the pin chases the floor in about a day. Both halves are load-bearing — `minimumReleaseAge` bounds when a release becomes *eligible*, never when Renovate *runs*, and the inherited `schedule:weekly` resolves through `schedule:earlyMondays` to `* 0-3 * * 1`, so the cooldown alone is a rule that looks applied and still waits a week. Verified by running Renovate's own `applyPackageRules` and `pep440` rather than reading its docs: **appended** the rule resolves to 1 day, **prepended** the generic pypi rule overrides it back to 3, `0.16→0.17` classifies as `minor`, and `0.16→1.0.0` stays `major` on dashboard approval. Scoped by **file**, because the `ruff>=0.15` floor in `template/pyproject.toml.jinja` reports the same depName and **ships into every generated service** — it keeps the full 3-day supply-chain cooldown. A pin-equals-resolved CI check was rejected deliberately: on a stale pin it reds *every* PR for close to a week, and noise that trains people to ignore red costs more than a lag that announces itself. The exposure is **narrower than filed** — 0.15.0 and 0.16.0 format this template's Python identically (47 leg files, and a 0.4.10-mangled variant of them), so what diverges across that window is file *discovery*, measured as a one-release-stale pin checking **35** files green where the leg checked **43** and failed one |
...
| — | The repo's **own** tooling is linted, not just the code it generates | **policy/CI** | `.github/workflows/ci.yml` job `repo-lint`, in `ci-ok`'s `needs`. Every other ruff invocation in that file runs with `working-directory: /tmp/generated`, so the manager repo's own Python had **no gate at all** and two files had already drifted — including `micro-render-check.py`, a guardrail script. Scoped to `scripts/` deliberately: `template/` is checked by the capability legs under its *own* `[tool.ruff]`, and root config reports 22 findings there that the template configures away, so an unscoped gate would make two gates disagree about one file. The ruff version is pinned — a floating linter once turned three legs red on a release, and a gate that changes verdict without a commit is not a gate. The pin is read from `.ruff-version` rather than written inline here; the row below is why |
